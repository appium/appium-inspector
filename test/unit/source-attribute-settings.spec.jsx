...
describe('Saved source attribute settings', () => {
  ...
  it('initializes the first session from the shared default without changing its order', async () => {
    const {DEFAULT_SETTINGS, IMPORTANT_SOURCE_ATTRS} = await import('../../app/common/shared/setting-defs.js');
    const originalDefaults = [...IMPORTANT_SOURCE_ATTRS];
    renderSession(await loadWrapper());

    expect(rendered.actions.importantAttrs).toEqual([...originalDefaults].sort());
    expect(DEFAULT_SETTINGS[settingKey]).toBe(IMPORTANT_SOURCE_ATTRS);
    expect(IMPORTANT_SOURCE_ATTRS).toEqual(originalDefaults);
    expect(renderedNode()).toContain('Email');
    expect(renderedNode()).not.toContain('Enter email');
  });

  it('loads and sorts saved attributes before rendering the first session', async () => {
    savedSettings.set(settingKey, ['text', 'hint']);
    renderSession(await loadWrapper());

    expect(rendered.actions.importantAttrs).toEqual(['hint', 'text']);
    expect(renderedNode()).toContain('Enter email');
    expect(savedSettings.get(settingKey)).toEqual(['text', 'hint']);
  });
  ...
  it('can restore the shared defaults after customization without mutating them', async () => {
    ...
    expect(rendered.actions.importantAttrs).toEqual([...originalDefaults].sort());
    expect(savedSettings.get(settingKey)).toEqual([...originalDefaults].sort());
    expect(IMPORTANT_SOURCE_ATTRS).toEqual(originalDefaults);
    expect(renderedNode()).toContain('Email');
    expect(renderedNode()).not.toContain('Enter email');
  });
