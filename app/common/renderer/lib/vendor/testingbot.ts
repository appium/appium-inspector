...
export class TestingbotVendor extends BaseVendor {
  override async configureProperties(): Promise<void> {
    ...
    const key = (testingbot.username as string | undefined) || (process.env.TB_KEY as string | undefined);
    const secret = (testingbot.accessKey as string | undefined) || (process.env.TB_SECRET as string | undefined);
